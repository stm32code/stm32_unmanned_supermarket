...
//        userExample.createCriteria().andIdEqualTo(1);
    ...
    @Override
    public PageInfo<BsUser> getUserListByCondition(String uname, String phone, Integer page, Integer limit) {
        PageHelper.startPage(page,limit);
        BsUserExample example = new BsUserExample();
    ...
    @Override
    public BsUser getUserByRfid(String cardId) {
        BsUserExample example = new BsUserExample();
        example.createCriteria().andRfidEqualTo(cardId);
        List<BsUser> bsUsers = userMapper.selectByExample(example);
