...
//	char data;
    ...
    printf("ID>>>%s\r\n", num);
		...
		sprintf( (char *) p, "GET http://192.168.1.11:9999/user/puttRfid?rfidId=%s", num );      
	  ...
	  printf("\n\r %s \n\r",cRes1);
		...
		showStr = 1;
				...
				OLED_Fill( 0x00 );				
			  OLED_ShowStr( 0, 2, cRes1, 2 ); 
			  OLED_ShowStr( 0, 5, cRes2, 2 );  
		memset(USART2_RX_BUF,0,sizeof(USART2_RX_BUF)/sizeof(char));
		memset(cRes,0,sizeof(cRes)/sizeof(char));
		memset(cRes1,0,sizeof(cRes1)/sizeof(char));
		memset(cRes2,0,sizeof(cRes2)/sizeof(char));
